...
            // Always use canonical YouTube watch URL
...
                        // Extract channel handle (@username) using main regex only
                        let channelHandle = this.extractMatch(videoPageBody, CHANNEL_HANDLE_REGEX);
                        if (!channelHandle) {
                            // Try canonical link pattern: <link rel="canonical" href="https://www.youtube.com/@username">
                            const canonicalLinkMatch = videoPageBody.match(/<link rel="canonical" href="https:\/\/www\.youtube\.com\/@([a-zA-Z0-9_.-]+)/);
                            if (canonicalLinkMatch && canonicalLinkMatch[1]) {
                                channelHandle = canonicalLinkMatch[1];
                            }
                        }
                        if (!channelHandle) {
                            // Fallback: find any /@username in the HTML
                            const genericHandleMatch = videoPageBody.match(/\/@([a-zA-Z0-9_.-]{3,})/);
                            if (genericHandleMatch && genericHandleMatch[1]) {
                                channelHandle = genericHandleMatch[1];
                            }
                        }
                        channelHandle = channelHandle || channelId || '';
                        const publishDateMatch = videoPageBody.match(PUBLISH_DATE_REGEX);
